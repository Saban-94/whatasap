...
export default function ChatPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { id: "err", role: "bot", text: "תקלה בחיבור. וודא מפתחות API ב-Vercel." }]);
    } finally { setIsSearching(false); }
  };

  return (
    <div className="flex h-screen bg-[#F0F2F5] overflow-hidden font-heebo" dir="rtl">
      <div className="flex-1 flex flex-col relative bg-[#EFE7DE]">
        {/* תמונת רקע עם Z-Index נמוך מאוד שלא תחסום לחיצות */}
        ...
        <header className="h-16 bg-[#F0F2F5] border-b flex items-center px-6 shadow-sm z-10">
          <h1 className="font-black text-emerald-800 text-xl">ח. סבן - יועץ AI</h1>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-28 z-10 custom-scroll">
          {messages.map((m) => (
            <div key={m.id} className={`flex flex-col ${m.role === "user" ? "items-start" : "items-end"}`}>
              {/* צבע טקסט שחור מפורש למניעת שקיפות */}
              <div className={`p-4 rounded-2xl max-w-[85%] shadow-md border ${m.role === "user" ? "bg-[#D9FDD3] text-[#111B21]" : "bg-white text-[#111B21]"}`}>
                ...
                {m.product && (
                  <ProductCard 
                    {...m.product} 
                    onWatchTutorial={() => setVideo(m.product.tutorialUrl)} 
                    onAddToOrder={(q:any) => setOrder(prev => [...prev, { ...m.product, qty: q }])} 
                  />
                )}
              </div>
            </div>
          ))}
          {isSearching && <div className="text-emerald-700 font-black text-xs animate-bounce">מעבד נתונים...</div>}
          <div ref={chatEndRef} />
        </div>

        <footer className="p-4 bg-[#F0F2F5] border-t z-20 flex gap-3">
          <input 
            value={input} 
            onChange={e => setInput(e.target.value)} 
            onKeyDown={e => e.key === "Enter" && handleSend()} 
            className="flex-1 p-3 rounded-xl border border-gray-300 outline-none font-bold text-black bg-white" 
            placeholder="חפש מוצר..." 
          />
          <button onClick={handleSend} className="bg-[#00a884] text-white p-3 rounded-full shadow-lg active:scale-90 transition-all">
            <Send size={24} />
          </button>
        </footer>
        ...
        {video && (
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl overflow-hidden w-full max-w-lg shadow-2xl relative">
              <button onClick={() => setVideo(null)} className="absolute top-4 right-4 z-10 bg-white/80 rounded-full p-2 text-black"><X /></button>
