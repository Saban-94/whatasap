...
import { Send, HardHat, Camera, FileUp, ShoppingCart, Play, Loader2, User } from 'lucide-react';

// הגדרת סוגי הנתונים למניעת שגיאות Build
interface Product {
  id: number;
  name: string;
  category: string;
...
const ProductCard = ({ product, onOrder }: { product: Product, onOrder: (name: string) => void }) => (
  <div className="mt-4 bg-[#162127] rounded-2xl overflow-hidden border border-gray-700 shadow-2xl animate-in zoom-in-95 duration-500 max-w-sm mr-auto ml-0">
...
export default function CustomerAiPage() {
  const [messages, setMessages] = useState<Message[]>([
    { role: 'assistant', content: 'שלום רמי אחי! המערכת עודכנה במוצרי סיקה. צלם לי את הסדק או העלה מפרט, ואני אגיד לך בדיוק מה להזמין.', product: null }
  ]);
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      setMessages(prev => [...prev, { role: 'user', content: `העליתי קובץ/צילום: ${file.name}`, product: null }]);
      setIsThinking(true);
      ...
      setTimeout(() => {
        ...
        const recommendedProduct: Product = { 
            ...
            name: "Sikadur 52", 
            category: "אפוקסי", 
            description: "שרף אפוקסי נוזלי להזרקה לאיטום סדקים.", 
            application_method: "הזרקה בלחץ", 
            unit: "ערכת 3 ק\"ג", 
        ...
        setMessages(prev => [...prev, { 
          role: 'assistant', 
          content: 'מניתוח הצילום, מומלץ להשתמש ב-Sikadur 52 לאיטום הסדק המבני.',
  ...
  return (
    <div className="flex flex-col h-screen bg-[#0b141a] text-right" dir="rtl">
      ...
      <div className="bg-[#202c33] p-4 flex items-center justify-between border-b border-gray-700 shadow-2xl z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[9px] text-green-500 font-black tracking-widest uppercase italic">VIP Engineering Service</p>
          </div>
        </div>
      </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-6 bg-[url('https://user-images.githubusercontent.com/15075759/28719144-86dc0f70-73b1-11e7-911d-60d70fcd2de8.png')] bg-repeat opacity-95">
        {messages.map((m, i) => (
          <div key={i} className={`flex flex-col ${m.role === 'user' ? 'items-start' : 'items-end'} animate-in fade-in slide-in-from-bottom-2`}>
            <div className={`flex gap-3 max-w-[85%] ${m.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
              <div className={`p-4 rounded-2xl shadow-xl ${m.role === 'user' ? 'bg-[#005c4b] text-white rounded-tr-none' : 'bg-[#202c33] text-white rounded-tl-none border border-gray-700'}`}>
        ...
        {isThinking && (
          <div className="flex justify-end p-2 animate-pulse text-[10px] text-gray-400 font-bold italic tracking-widest">
            ג'ימיני מנתח מפרטים טכניים...
          </div>
        )}
      </div>
