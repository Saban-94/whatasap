...
export default function InventoryPage() {
  ...
  const fetchInventory = async () => {
    setLoading(true)
    try {
      // שליפת העמודות המדויקות לפי קובץ ה-SQL שהעלית
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto min-h-screen bg-[#fbfbfb]" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-6 rounded-3xl shadow-sm border border-slate-100 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-[#0B2C63] tracking-tight">מלאי מחסן סבן</h1>
            <p className="text-[10px] font-bold text-green-600 uppercase tracking-widest mt-1">חיבור ישיר למסד הנתונים</p>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          <Button variant="outline" onClick={fetchInventory} disabled={loading} className="rounded-xl border-slate-200 font-bold h-11">
      ...
      <div className="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
        {loading && items.length === 0 ? (
          <div className="flex flex-col justify-center items-center p-24 gap-4">
            ...
            <p className="text-slate-400 font-black text-[10px] uppercase tracking-[4px]">Fetching Saban Data...</p>
          </div>
        ) : (
            ...
            <table className="w-full text-right border-collapse">
              <thead>
                <tr className="bg-slate-50/50 border-b border-slate-100">
                  ...
                  <th className="px-6 py-5 font-black text-[#0B2C63] text-sm text-center">מחלקה</th>
                  <th className="px-6 py-5 font-black text-[#0B2C63] text-sm">ספק</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {filteredItems.length === 0 ? (
                  <tr><td colSpan={5} className="py-20 text-center text-slate-400 font-bold">לא נמצאו מוצרים תואמים</td></tr>
                ) : (
                  filteredItems.map((item, idx) => (
                    <tr key={idx} className="hover:bg-blue-50/20 transition-colors group">
                      <td className="px-6 py-5 font-black text-slate-800 leading-tight">
                        {item.product_name}
                      </td>
                      <td className="px-6 py-5 font-mono text-[11px] font-bold text-blue-600/70 tracking-tighter">
                        {item.sku || '---'}
                      </td>
                      <td className="px-6 py-5">
                        <span className="bg-blue-50 text-blue-700 px-3 py-1.5 rounded-full font-black text-[9px] uppercase flex items-center gap-1.5 w-fit border border-blue-100">
                      ...
                      <td className="px-6 py-5 text-sm font-bold text-slate-500">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="truncate max-w-[150px]">{item.supplier_name || 'ספק סבן'}</span>
                        </div>
                      </td>
                    </tr>
                  ))
      ...
      <div className="flex justify-center pt-4">
        <p className="text-[9px] font-black text-slate-300 uppercase tracking-[6px] italic">Saban OS Intelligence Layer v3.1</p>
      </div>
    </div>
  )
}
