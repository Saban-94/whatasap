...
import { Search, Send, Zap, Loader2, MessageSquare, Database, RefreshCw, Smartphone } from 'lucide-react';
import { processSmartOrder } from '@/lib/dataEngine'; // המוח המקצועי ששודרג ל-3.1 Pro
...
export default function SabanAICanvasMobile() {
  ...
  useEffect(() => {
    setMounted(true);
    // הגדרת חוויית App: מניעת גלילה של הרקע במובייל
    document.body.style.overflow = 'hidden';
    return () => { document.body.style.overflow = 'auto'; };
  }, []);

  // גלילה אוטומטית לתשובה חדשה
  useEffect(() => {
    if (response) {
  ...
  const handleAction = async () => {
    ...
    // סגירת מקלדת במובייל לשיפור הויזואליות
