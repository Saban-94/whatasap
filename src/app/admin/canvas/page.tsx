...
export default function SabanAICanvasMobile() {
    ...
    <div className="fixed inset-0 bg-[#0b141a] text-white flex flex-col font-sans overflow-hidden select-none" dir="rtl">
      ...
      <header className="h-16 border-b border-gray-800/60 flex items-center justify-between px-6 bg-[#111b21]/95 backdrop-blur-2xl z-[100] shadow-xl">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 bg-[#00a884] rounded-xl flex items-center justify-center font-black text-lg shadow-lg">ס</div>
        ...
        <div className="flex items-center gap-4 text-gray-500 font-bold text-[10px]">
           {response && (
             <button onClick={() => setResponse(null)} className="p-2 bg-gray-800/50 rounded-full active:scale-90 transition-transform">
               <RefreshCw size={14} />
             </button>
           )}
           <span className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 bg-[#00a884] rounded-full animate-pulse" /> LIVE</span>
        </div>
      </header>
      ...
      <main className="flex-1 relative overflow-y-auto p-5 pb-36 touch-pan-y">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none opacity-60">
            <div className="relative w-64 h-64 flex items-center justify-center">
              ...
              <div className="w-40 h-40 bg-gradient-to-tr from-[#00a884] to-[#0b141a] rounded-full shadow-[0_0_60px_rgba(0,168,132,0.3)] flex items-center justify-center border-4 border-[#111b21] animate-pulse">
                <img src="/avattar.png" className="w-24 h-24 object-contain" alt="Avatar" />
              </div>
            </div>
              ...
              <div key={item.id} className="bg-[#111b21] rounded-3xl border border-gray-800 overflow-hidden flex flex-col shadow-lg">
                ...
                <div className="p-4 flex justify-between items-center bg-[#1c272d]">
                  <div className="space-y-1">
                    ...
                    <p className="text-[10px] text-gray-500 uppercase font-mono">SKU: {item.sku}</p>
                  </div>
                  <div className="text-left">
                    <p className="text-[#00a884] font-black text-lg">₪{item.price}</p>
                    <button className="text-[10px] bg-[#00a884] text-white px-4 py-2 rounded-full font-bold mt-1 active:scale-90 transition-transform shadow-lg shadow-[#00a884]/20">הוסף להזמנה</button>
                  </div>
                </div>
              </div>
            ))}
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-20 animate-pulse">
            ...
            <p className="text-[10px] font-black text-[#00a884] uppercase tracking-widest">סורק נתונים...</p>
          </div>
        )}
      </main>
...
}      // שליפה מהמוח המקצועי המעודכן ל-Gemini 3.1 Pro
      const result = await processSmartOrder(query);
      setResponse(result);
    } catch (e) {
      console.error("AI Interface Error:", e);
  ...
  return (
    <div className="fixed inset-0 bg-[#0b141a] text-white flex flex-col font-sans overflow-hidden select-none" dir="rtl">
      ...
      <header className="h-16 border-b border-gray-800/60 flex items-center justify-between px-6 bg-[#111b21]/95 backdrop-blur-2xl z-[100] shadow-xl">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 bg-gradient-to-br from-[#00a884] to-[#005c4b] rounded-xl flex items-center justify-center shadow-lg shadow-[#00a884]/20">
            <span className="font-black text-lg">ס</span>
          </div>
          <div className="flex flex-col">
            <h1 className="text-sm font-black tracking-tighter leading-none uppercase">Saban AI Canvas</h1>
            <span className="text-[8px] text-[#00a884] font-bold tracking-[0.2em] mt-1">MOBILE ENGINE 3.1</span>
          </div>
        </div>
      ...
      <main className="flex-1 relative overflow-y-auto p-5 pb-36 touch-pan-y">
        ...
        {/* The 720° Rotating Orb - הלב של האפליקציה */}
        {!response && !loading && (
          <div className="absolute inset-0 flex flex-col items-center justify-center opacity-80">
            <div className="relative w-64 h-64 flex items-center justify-center">
              {/* טבעת 3D חיצונית מסתובבת 720 מעלות */}
              <div className="absolute inset-0 border-[2px] border-dashed border-[#00a884]/30 rounded-full animate-spin-720" />
              <div className="absolute inset-6 border border-[#00a884]/10 rounded-full animate-spin-reverse" />
              ...
              <div key={item.id} className="bg-[#111b21] rounded-[2.5rem] border border-gray-800 overflow-hidden flex flex-col shadow-2xl animate-in fade-in zoom-in duration-500">
                ...
                <div className="p-6 bg-[#1c272d] flex justify-between items-center border-t border-gray-800">
                  <div className="space-y-1">
                    ...
                    <p className="text-[10px] text-gray-500 font-bold">מק"ט: {item.sku}</p>
                  </div>
                  <div className="text-left">
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-10 space-y-4">
             ...
             <p className="text-[10px] font-black text-[#00a884] animate-pulse uppercase tracking-widest">סורק מלאי וידע טכני...</p>
          </div>
        )}
      </main>

      {/* Fixed Mobile Input - The Interaction Core */}
      <div className="p-5 bg-gradient-to-t from-[#0b141a] via-[#0b141a] to-transparent absolute bottom-0 w-full z-[200]">
        <div className="bg-[#1c272d] border border-gray-700/50 rounded-[2.5rem] p-1.5 flex items-center shadow-[0_-10px_40px_rgba(0,0,0,0.5)] focus-within:border-[#00a884] transition-all duration-300">
          <div className="p-4 text-gray-500">
          ...
          <input 
            ...
            onChange={(e) => setQuery(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && handleAction()}
            placeholder="חשב לי כמה קרטונים ל-20 מטר..." 
            className="flex-1 bg-transparent outline-none py-4 text-[15px] font-medium placeholder:text-gray-600"
          />
        ...
        <div className="flex justify-center gap-4 mt-4 mb-2 overflow-x-auto no-scrollbar py-1 px-4">
          {['סיקה 107', 'מחירון VIP', 'יתרת מלאי'].map(tag => (
            <button 
              key={tag}
              onClick={() => setQuery(tag)}
              className="whitespace-nowrap bg-gray-900/50 border border-gray-800 text-gray-500 text-[9px] font-black px-4 py-2 rounded-full active:bg-[#00a884]/10 active:text-[#00a884] transition-colors"
            >
              {tag}
            </button>
          ))}
