...
export default function BusinessInfoManager() {
  ...
  async function fetchData() {
    setLoading(true);
    try {
      // שימוש בשם הטבלה המדויק business_info למניעת 404
      ...
      if (error) {
        ...
        toast.error(`שגיאה: ${error.message}. וודא שהטבלה business_info קיימת.`);
      } else {
        setItems(data || []);
      }
    } catch (err: any) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(editingId ? "המידע עודכן" : "מידע חדש נוסף לזיכרון ה-AI");
      resetForm();
      fetchData();
    } catch (error: any) {
      ...
      <header className="max-w-5xl mx-auto mb-10 flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-2 font-bold uppercase tracking-widest">
            ח. סבן AI - הגדרת שעות, סניפים ומדריכים
          </p>
        </div>
        <button onClick={fetchData} className="p-3 bg-white/5 rounded-2xl hover:bg-white/10 transition-all">
      ...
      <div className="max-w-5xl mx-auto bg-slate-900/40 border border-white/5 p-8 rounded-[40px] mb-12 backdrop-blur-xl shadow-2xl">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          <div className="md:col-span-2">
            <label className="block text-[10px] font-black text-slate-500 mb-2 mr-2 uppercase italic">נושא המידע (שאלה)</label>
            <input 
        ...
        <div className="mb-6">
          <label className="block text-[10px] font-black text-slate-500 mb-2 mr-2 uppercase italic">התשובה שג'ימיני יספק</label>
          <textarea 
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <MediaInput icon={<ImageIcon size={16}/>} placeholder="URL לתמונה" value={form.image_url} onChange={val => setForm({...form, image_url: val})} color="text-blue-400" />
          <MediaInput icon={<FileText size={16}/>} placeholder="URL ל-PDF" value={form.file_url} onChange={val => setForm({...form, file_url: val})} color="text-red-400" />
          <MediaInput icon={<Youtube size={16}/>} placeholder="URL ליוטיוב" value={form.video_url} onChange={val => setForm({...form, video_url: val})} color="text-red-600" />
        </div>

        <div className="flex gap-4">
          <button onClick={handleSave} className="flex-1 bg-blue-600 hover:bg-blue-500 py-4 rounded-3xl font-black text-xl flex items-center justify-center gap-2 transition-all shadow-lg shadow-blue-900/20 active:scale-95">
            <Save size={24} /> {editingId ? "עדכן זיכרון" : "שמור בזיכרון ה-AI"}
          </button>
          {editingId && (
            <button onClick={resetForm} className="px-8 bg-slate-800 hover:bg-slate-700 rounded-3xl font-bold transition-all">
              ביטול
            </button>
          )}
        </div>
      ...
      <div className="max-w-5xl mx-auto space-y-4">
        {items.length === 0 && !loading && (
          <div className="text-center p-20 border-2 border-dashed border-white/5 rounded-[40px] opacity-30 italic font-bold">
            ...
            אין מידע עסקי רשום. ה-AI ישתמש במידע הכללי שלו בלבד.
          </div>
        )}
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                ...
                <div className="flex gap-2">
                  {item.image_url && <ImageIcon size={14} className="text-blue-500/50" />}
                  {item.file_url && <FileText size={14} className="text-red-500/50" />}
                  {item.video_url && <Youtube size={14} className="text-red-600/50" />}
                </div>
              </div>
      ...
      {loading && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-xl flex items-center justify-center z-50">
          <div className="text-center">
            ...
            <p className="font-black italic tracking-widest text-blue-500">SABAN AI UPDATING...</p>
          </div>
        </div>
      )}
    </div>
...
function MediaInput({ icon, placeholder, value, onChange, color }: any) {
  return (
